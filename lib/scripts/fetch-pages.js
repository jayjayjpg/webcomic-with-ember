...
module.exports = function fetchAllPages(jobName, url) {
  ...
  function fetchPages(numOfPages) {
    console.log("Fetch Pages....✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨");
      function excludes(blob) {
        return !blob.name.includes('beta') &&
        !blob.name.includes('rc') &&
        !blob.name.includes('alpha') &&
        !blob.name.includes('ngcontainer') &&
        !blob.name.endsWith('.10') &&
        !blob.name.endsWith('.20') &&
        !blob.name.endsWith('.30') &&
        blob.name.endsWith('0');
      }
      getPagesData(numOfPages)
        .then(function(results) {
          let dataPoints = [];
          results.forEach((res) => {
            let filteredRes = res.filter(excludes)
            ...
            getTagData(dataPoints).then((tagData) => {
              let fullData = tagData.map((data) => {
                ...
                let tagMod = tag.replace(/[a-zA-Z|\-\_]/g, '').replace(/\.0$/g, '');
                return { y: tagMod, x: data.commit.author.date, url: data.url, author: data.commit.author.name };
              });

              fs.writeFile(`lib/scripts/data/${jobName}.json`, JSON.stringify(fullData, null, 4), function(err) {
                  if(err) {
